...
Given an **array A** containing N elements. Partition any subset of this array
into two disjoint subsets such that both the subsets have an identical sum.
Obtain the maximum sum that can be obtained after partitioning.

 _Note:_ It is not necessary to partition the entire array, that is any
element might not contribute to any of the partition.
...
>  **Input:** A = [1, 2]  
>  **Output:** 0  
>  **Explanation:** No subset can be partitioned into 2 disjoint subsets with
> identical sum
 ...
 **Naive Approach:**  
The above problem can be solved by **brute force method using recursion**. All
the elements have three possibilities. Either it will contribute to partition
1 or partition 2 or will not be included in any of the partitions. We will
perform these three operations on each of the element and proceed to the next
element in each recursive step.
...
**Time Complexity:** ![O\(3^n\)](https://www.geeksforgeeks.org/wp-content/ql-
cache/quicklatex.com-7f6ecd4d2b2269ad6b2a6e520d4bdb9b_l3.png)
...
>
>
> the difference between the sum of g0 and g1 is (sum of all elements – j),
> where j is the difference.
>
> So, the answer would be dp[n][sum]

Now we might encounter, the difference between the sums is negative, lying in
the range [-sum, +sum], where the _sum_ is a summation of all elements. The
minimum and maximum ranges occurring when one of the subsets is empty and the
other one has all the elements. Due to this, in the DP state, we have defined
j as (sum – diff). Thus, j will range from [0, 2*sum].
...
INT_MIN) 
 ...
 if ((j + a[i - 1]) < limit && dp[i - 1][j + a[i - 1]] !=
INT_MIN) 
...
a[i - 1]] != INT_MIN) :
 ...
 if ((j + a[i - 1]) < limit and dp[i - 1][j + a[i
- 1]] != INT_MIN) :
...
**Time Complexity:** ![O\(N*Sum\)](https://www.geeksforgeeks.org/wp-
content/ql-cache/quicklatex.com-c8a3d0ccb7115af3281ba4e0a1476651_l3.png),
where Sum represents sum of all array elements.  
 **Auxiliary Space:** ![O\(N*Sum\)](https://www.geeksforgeeks.org/wp-
content/ql-cache/quicklatex.com-c8a3d0ccb7115af3281ba4e0a1476651_l3.png)
