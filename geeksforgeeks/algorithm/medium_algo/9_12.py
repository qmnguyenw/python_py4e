...
Given an array **arr[]** , the task is to find the sum of the minimum elements
...
A **simple method** is to generate all the sub-arrays and then sum the minimum
...
encounter an element **strictly lesser** than **arr[i]** or we don’t reach the
left end of the array. Let us call this count for the index **i** of the given
array as **CLi**.  
 **Right count:** We iterate towards right of the index till we don’t
encounter an element **less than or equal to** the value at the index or we
...
index **i** in which its value will be minimum because there are **CLi + 1**
...
the index **i** and the right most element whose value is strictly less than
**arr[i]** having index less than **i**. If, there is no such element, then
**CLi** for an element will be equal to the number of elements to the left of
the index **i**.
...
stack. While the value **arr[i]** is **less than or equal to** the value at
...
traversing right to left in the array. While **arr[i]** is **strictly less**
than the value at the top most index in the stack and the stack is not empty,
keep popping the elements. Whenever, an element is popped, the right count of
the current index(i) is updated as **CRi = CRi + CRj + 1**.
