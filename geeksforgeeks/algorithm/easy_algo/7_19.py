...
> 4 5  
>  **Output:** 10  
> ![](https://media.geeksforgeeks.org/wp-content/cdn-uploads/Screen-
> Shot-2018-06-22-at-1.11.51-PM.png)
>
>  
>
>
 ...
 **Approach:** Finding connected components for an undirected graph is an
easier task. Doing either a BFS or DFS starting from every unvisited vertex
will give us our connected components. Create a **visited[]** array which has
initially all nodes marked as False. Iterate all the nodes, if the node is not
visited, call **DFS()** function so that all the nodes connected directly or
indirectly to the node are marked as visited. While visiting all the directly
or indirectly connected nodes, store the minimum value of all nodes. Create a
variable **sum** which stores the summation of the minimum of all these
connected components. Once all the nodes are visited, _sum_ will have the
