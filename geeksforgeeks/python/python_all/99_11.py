...
contains numbers which are odd. This particular utility has an application in
...
then testing the starting element of string and if they are odd we can return
...
res = len(set( not(int(sub[0]) % 2) for sub
...
checking for all elements with the first digit of first element to be odd.
...
res = all(int(str(i)[0]) % 2 for i in
test_list)
