...
which could give the position to each element destined if the list was sorted.
This has a good application in web development and competitive programming
...
sorted list is formed and the index of actual ordering in sorted list are
traversed using enumerate to have key-value pair and then are get by map for
all the indices in list.
...
temp = {val: key for key, val in
enumerate(sorted(test_list))}
