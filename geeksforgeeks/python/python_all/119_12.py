...
in which we need to find the maximum element of all the records received. This
...
iterate over whole nested lists using generator expression and get the maximum
element using max().
...
**Method #2 : Usingmax() + map() + chain.from_iterable()**  
The combination of above methods can also be used to perform this task. In
this, the extension of finding maximum is done by combination of map() and
from_iterable().
