...
records and we require to find the highest N scores from it. This kind of
...
task. In this, we just employ sorted function with reverse flag true, and
print the top N elements using list slicing.
...
res = sorted(test_list, key = lambda x: x[1], reverse =
True)[:N]
...
res = sorted(test_list, key = itemgetter(1), reverse =
True)[:N]
