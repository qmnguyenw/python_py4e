...
summation of 2 numbers from Strings but with a constraint of having the
numbers in successions. This type of problem can occur while competitive
...
which max function can be used to get the max value, zip and list
...
res = max(int(a) + int(b) for a, b in
...
**Method #2 : Usingmax() + map() + operator.add**  
...
the logic to whole list and add operator is used to perform the addition.
...
res = max(map(add, map(int, test_string), map(int,
test_string[1:])))
