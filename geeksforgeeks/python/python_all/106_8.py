...
which we need to extract just specific sized strings above a minimum
threshold. This kind of problem can occur during validation cases across many
domains. Let’s discuss certain ways to handle this in Python strings list.

 **Method #1 : Using list comprehension +len()**  
The combination of above functionalities can be used to perform this task. In
this, we iterate for all the strings and return only above threshold strings
checked using len().
...
res = list(filter(lambda ele: len(ele) >= thres,
test_list))
