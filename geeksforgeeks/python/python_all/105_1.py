...
to find all records between two lists that we receive. This is a very common
problem and records usually occurs as a tuple. Let’s discuss certain ways in
which this problem can be solved.

 **Method #1 : Using list comprehension**  
List comprehension can be opted as method to perform this task in one line
rather than running a loop to find the union elements. In this, we just
iterate for single list and check if any element occurs in other one. If no,
we again populate the made list.
...
res2 = [ele2 for ele2 in test_list2 if ele2 not in
res1]
...
This task can also be performed in smaller way using the generic set union. In
this, we first convert the list of records to a set and then perform its union
using union().
