...
product of 2 numbers from list but with a constraint of having the numbers in
...
**Method #2 : Usingmax() + map() + operator.mul**
...
the logic to whole list and mul operator is used to perform the
multiplication.
...
res = max(map(mul, map(int, test_string), map(int,
test_string[1:])))
