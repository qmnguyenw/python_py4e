...
select some of the elements that occur range times consecutively. This problem
...
the numbers that are occurring range consecutively. We iterate the list using
list comprehension.
...
res1 = [i for i, j in groupby(test_list) if
len(list(j)) <= end]

res2 = [i for i, j in groupby(test_list) if
len(list(j)) >= strt]
...
res1 = [i for i, j in groupby(test_list) if
len(list(islice(j, 0, strt))) >= strt]

res2 = [i for i, j in groupby(test_list) if
len(list(islice(j, 0, end))) <= end]
