...
from N lists and return their sum. Let’s discuss certain ways in which this
operation can be performed.

 **Method #1 : Usingreduce() + lambda + set() + sum()**  
...
convert list into a set to remove repetition. The task of performing sum is
done using sum().
...
res = sum(list(reduce(lambda i, j: i & j, (set(x)
for x in test_list))))
...
**Method #2 : Usingmap() + intersection() + sum()**  
...
performing sum is done using sum().
...
res = sum(list(set.intersection(*map(set,
test_list))))
