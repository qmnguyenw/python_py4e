...
require to perform index wise maximum of tuple elements. This can get
...
we combine the elements across tuples using zip(). The iterations and maximize
...
**Method #2 : Usingisinstance() + zip() + max() \+ loop + list
comprehension**  
...
res = tuple(tup_max(x, y) for x, y in zip(test_tup1,
test_tup2))
