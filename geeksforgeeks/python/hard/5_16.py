...
Pygame is a python library that can be used specifically to design and build
games. Pygame supports only 2d games that are built using different sprites.
Pygame is not particularly best for designing games as it is very complex to
use doesn’t have a proper GUI like unity but it definitely builds logic for
further complex projects.
...
We’ll be using various techniques such as the use of functions, random
variables, various pygame functions etc.

#### Installation

Before initializing pygame library we need to install it. To install it type
...
After installing the pygame library we need to write the following lines to
...
These lines are pretty self explanatory. The pygame.init() function
initiates the pygame library.

Then we need to initialize the screen where we want to place our blocks. This
can be done by writing following lines:-
...
The tuple res holds two values that will define the resolution of our game.
Then we need to define another variable screen that will actually act as our
workbench. This can be done by using pygame.display.set_mode((arg, arg)).
The tuple (arg, arg) can be stored into a variable res to reduce processor
load.

Now we need an actual screen to pop up when we run the code this can be done
by a for and while loop in following way:-
...
The while loop used here run till the condition is true. We define a variable
ev that in pygame.event. Now if the user clicks on the cross button of the
window the condition is changed to false and the while loop ends killing the
current window.
...
e1_p = [random.randint(width, width + 100), random.randint(50,
height
...
def game_over():
 ...
 if 100 < mouse1[0] < 140 and height - 100 <
mouse1[1] \
...
mouse1[1] < height - 80:
 ...
 pygame.draw.rect(screen, startl, [100, height - 100,
40,20])

 else:

 pygame.draw.rect(screen, startd, [100, height - 100,
40,20])
...
- 100 < mouse1[1] < height - 80:
 ...
 pygame.draw.rect(screen, startl, [width - 180, height- 100,
80, 20])

 else:

 pygame.draw.rect(screen, startd, [width - 180, height- 100,
80, 20])
...
pygame.draw.rect(screen, startd, [100, height - 100, 40,
20])
...
def game(
 ...
 rect = pygame.draw.rect(screen, player_c, [lead_x, lead_y,
40,40]) 
 ...
 pygame.draw.rect(screen, startd, [width - 100, 0, 100,
40])
 ...
 if width - 100 < mouse[0] < width and 0 < mouse[1] <
40:

 pygame.draw.rect(screen, startl, [width - 100, 0, 100,
40])
...
40:
 ...
 if lead_x <= e_p[0] <= lead_x + 40 and lead_y >=
e_p[1] >= lead_y - 40:
 ...
 if lead_y <= e_p[1] + enemy_size <= lead_y + 40
and lead_x <= e_p[0] <= lead_x + 40:
 ...
 pygame.draw.rect(screen, red, [e_p[0], e_p[1],
enemy_size,enemy_size])
 ...
 if lead_x <= e1_p[0] <= lead_x + 40 and lead_y >=
e1_p[1] >= lead_y - 40:
 ...
 if lead_y <= e1_p[1] + enemy_size <= lead_y + 40
...
def intro(
 ...
 if x < mouse[0] < x + width1 and y < mouse[1] < y +
height1:
 ...
 if x < mouse[0] < x + width1 + 40 and y + 70 <
mouse[1] < y \

 + 70 + height1:

 pygame.draw.rect(screen, startl, [x, y + 70,
width1+40,height1])
 ...
 if x < mouse[0] < width1 + x and y + 140 <
mouse[1] < y + 140 + height1:
...
mouse[1] < y + 140 + height1:
 ...
 if 0 <= colox_c1 <= 254 or 0 <= colox_c2 <=
254:
 ...
 sig = smallfont.render('Designed by :- Antriksh', True,
white)
...
content/uploads/20200117123314/media.io_Screencast-from-
Friday-17-January-2020-12_27_00-IST.mp4
